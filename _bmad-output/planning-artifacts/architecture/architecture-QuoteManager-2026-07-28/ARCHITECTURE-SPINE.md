...
- **Status:** backend contract implemented exactly as specified — `POST /api/auth/login` returns `{ accessToken, expiresAt, user: { id, displayName, roles, organizationId } }` (one additive field beyond the spec), 8-hour lifetime. `GET /api/auth/me` added beyond the original rule set, so an `AuthProvider` can rehydrate identity from a stored token without decoding the JWT client-side. The SPA half — `QuoteManager.Web`, `AuthProvider`, `apiClient`, the lint rules — does not exist yet; no client project has been scaffolded.
