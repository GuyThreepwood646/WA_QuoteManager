...
- **Status:** the closed role set, JWT role claims, and the affirmative row-level read posture are all implemented and now actually exercised — `GET /api/dashboard`, `/api/requests`, `/api/organizations` return every row to every authenticated role, matching the stated posture, no filter needed or added.
- **Gap found, not yet fixed:** the Vendor rule's second half — "quotes belonging to its own organisation" — is **not enforced**. `QuoteTransitions.Resolve`/`PermittedFor` check `actorRoles` only; `DomainActor` carries no `OrganizationId` at all, so nothing in `Request.ApplyQuoteAction` can compare the acting vendor's organisation to `Quote.VendorOrganizationId`. Any authenticated Vendor can currently `Submit` or `Withdraw` a *different* vendor's `Draft`/`Submitted` quote. Not visible by coincidence in the current seed (the one request with two active vendor quotes at once has the second stuck at `UnderReview`, a status no Vendor action reaches regardless of org) - found by manually testing as a second vendor, not by an existing test. Fixing it means threading `OrganizationId` onto `DomainActor` and adding an org check alongside the role check for Vendor-gated actions; deliberately not done inline here since it reshapes a type several call sites construct.
